...
class Chef::Provider::Service::Systemd < Chef::Provider::Service::Simple
  ...
  def enable_service
    shell_out_with_systems_locale!("#{systemctl_path} enable #{new_resource.service_name}")
  end

  def disable_service
    shell_out_with_systems_locale!("#{systemctl_path} disable #{new_resource.service_name}")
  end

  def is_active?
    shell_out_with_systems_locale("#{systemctl_path} is-active #{new_resource.service_name} --quiet").exitstatus == 0
  end

  def is_enabled?
    shell_out_with_systems_locale("#{systemctl_path} is-enabled #{new_resource.service_name} --quiet").exitstatus == 0
  end
