    ...
    class WindowsTask < Chef::Provider
      ...
      def task_needs_update?(task)
        ...
        if new_resource.frequency == :none
          ...
          if current_task_trigger.nil?
            ...
            flag  = ( start_day_updated?(current_task_trigger, new_task_trigger) == true ||
      ...
      def weeks_of_month
        ...
        if new_resource.frequency_modifier
          weeks = new_resource.frequency_modifier.split(",").map(&:upcase)
          weeks_of_month = get_constant_values(weeks, WEEKS_OF_MONTH)
        end
        weeks_of_month
      end
      ...
      def days_of_week
        if new_resource.day
          ...
          weeks_days = get_constant_values(days, DAYS_OF_WEEK)
        else
      ...
      def months_of_year
        ...
        if new_resource.months
          ...
          months_of_year = get_constant_values(months, MONTHS)
        else
      ...
      def get_constant_values(array_values, constant)
        data = []
