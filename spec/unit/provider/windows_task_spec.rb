...
describe Chef::Provider::WindowsTask do
  ...
  describe "#trigger_type" do
    ...
    it "raises error if no frequency values is set" do
      new_resource.frequency = nil
      expect{ provider.send(:trigger_type) }.to raise_error(ArgumentError, "Please set frequency")
    end
  end
  ...
  describe "#weeks_of_month" do
    ...
    it "returns the binary value 16 if frequency_modifier is set as 'last'" do
      ...
      expect(provider.send(:weeks_of_month)).to eq(16)
    end
  end
