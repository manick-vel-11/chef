  ...
  describe "on a host with /bin/svcs" do
    ...
    describe "when discovering the current service state" do
      it "should create a current resource with the name of the new resource" do
        allow(@provider).to receive(:shell_out_with_systems_locale!).with("/bin/svcs -l chef").and_return(@status)
      ...
      it "should call '/bin/svcs -l service_name'" do
        expect(@provider).to receive(:shell_out_with_systems_locale!).with("/bin/svcs -l chef", {:returns=>[0, 1]}).and_return(@status)
        @provider.load_current_resource
      end
    ...
    describe "when enabling the service" do
      ...
      it "should call svcadm enable -s chef" do
        expect(@provider).not_to receive(:shell_out_with_systems_locale!).with("/usr/sbin/svcadm clear #{@current_resource.service_name}")
        expect(@provider).to receive(:shell_out_with_systems_locale!).with("/usr/sbin/svcadm enable -s #{@current_resource.service_name}").and_return(@status)
      ...
      it "should call svcadm clear chef for start_service when state maintenance" do
        ...
        allow(@provider).to receive(:shell_out_with_systems_locale!).and_return(@status)
        @provider.load_current_resource
        expect(@provider).to receive(:shell_out_with_systems_locale!).with("/usr/sbin/svcadm clear #{@current_resource.service_name}").and_return(@status)
        expect(@provider).to receive(:shell_out_with_systems_locale!).with("/usr/sbin/svcadm enable -s #{@current_resource.service_name}").and_return(@status)
        expect(@provider.enable_service).to be_truthy
        expect(@current_resource.enabled).to be_truthy
      end
    ...
    describe "when disabling the service" do
      ...
      it "should call svcadm disable -s chef" do
        expect(@provider).to receive(:shell_out_with_systems_locale!).with("/usr/sbin/svcadm disable -s chef").and_return(@status)
